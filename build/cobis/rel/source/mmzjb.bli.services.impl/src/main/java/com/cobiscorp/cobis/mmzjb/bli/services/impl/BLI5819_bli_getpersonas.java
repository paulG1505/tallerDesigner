...
@Properties({
  ...
  @Property(name = "bli.id", value = "BLI5819_bli_getpersonas")
})
public class BLI5819_bli_getpersonas implements IBLIExecutor {
  private static final ILogger logger = LogFactory.getLogger(BLI5819_bli_getpersonas.class);
  ...
  public void execute(DynamicRequest dynamicRequest) {
    ...
    try {
      ...
      mapperSp1.addMapper(Persona.IDPERSONA, 1);
      mapperSp1.addMapper(Persona.NOMBRE, 2);
      mapperSp1.addMapper(Persona.APELLIDO, 3);
      mapperSp1.addMapper(Persona.TELEFONO, 4);
      ...
      if (ret && (retCode == 0)) {
        if (mapper.getResults().size() >= 1) {
          ...
          dynamicRequest.setEntityList(Persona.ENTITY_NAME, del1);
        }
