...
import com.cobiscorp.cobis.mmzjb.model.Prospecto;
...
@Properties({
  ...
  @Property(name = "bli.id", value = "BLI5278_bli_tipob")
})
public class BLI5278_bli_tipob implements IBLIExecutor {
  private static final ILogger logger = LogFactory.getLogger(BLI5278_bli_tipob.class);
  ...
  public void execute(DynamicRequest dynamicRequest) {
    ...
    try {
      ...
      mapper.addInputParameter("@i_operacion", SqlType.CHAR, "L");
      mapper.addInputParameter("@t_trn", SqlType.INT, "2515");
      if (dynamicRequest.getData().get("estado") != null){
        mapper.addInputParameter("@i_estado", SqlType.VARCHAR, BLIUtils.convertToType(dynamicRequest.getData().get("estado"), String.class));
      }
      ExecutorSP executorSP = new ExecutorSP(mapper);
      MapperResult mapperSp1 = new MapperResult();
      mapperSp1.addMapper(Prospecto.IDPROSPECTO, 1);
      mapperSp1.addMapper(Prospecto.NOMBRE, 2);
      mapperSp1.addMapper(Prospecto.APELLIDO, 3);
      mapperSp1.addMapper(Prospecto.TELEFONO, 4);
      mapperSp1.addMapper(Prospecto.ESTADO, 5);
      mapperSp1.addMapper(Prospecto.TIPO, 6);
      mapperSp1.addMapper(Prospecto.FECHA, 7, BLIUtils.getDateFormat(""));
      ...
      if (ret && (retCode == 0)) {
        if (mapper.getResults().size() >= 1) {
          ...
          dynamicRequest.setEntityList(Prospecto.ENTITY_NAME, del1);
        }
